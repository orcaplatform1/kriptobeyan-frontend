      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        ...
        <div className="mt-10 flex flex-wrap gap-3">
          {LIVE_EXCHANGES.map((name) => (
            <span
              key={name}
              className="rounded-full border border-gold/25 bg-cream px-4 py-2 text-sm font-medium text-ink"
            >
              {name}
            </span>
          ))}
          {UPCOMING_EXCHANGES.map((name) => (
            <span
              key={name}
              className="flex items-center gap-1.5 rounded-full border border-ink/10 bg-cream/60 px-4 py-2 text-sm font-medium text-ink-soft/70"
            >
              {name}
